...
export default function Onboarding({ onComplete, session, prefillInviteCode }) {
  ...
  async function handleCreateAccount() {
    ...
    const { data: invite } = await supabase
      .from('invites').select('id, used_by')
      .eq('code', inviteCode.trim().toUpperCase()).maybeSingle()

    if (!invite) { setAuthError('That invite code is invalid. Ask a friend for one.'); return }
    if (invite.used_by) { setAuthError('That invite code has already been used.'); return }
    ...
    if (existingUsername) { setAuthError('That username is already taken. Try another one.'); setAuthLoading(false); return }

    const { data: existingEmail } = await supabase
      .from('profiles').select('id').eq('email', email).maybeSingle()
    if (existingEmail) { setAuthError('An account with this email already exists. Try logging in instead.'); setAuthLoading(false); return }

    const { data, error } = await supabase.auth.signUp({ email, password })
    if (error) {
      setAuthError(error.message.toLowerCase().includes('already registered')
    ...
    // Insert profile — session is established by this point
    ...
    await supabase.from('invites').insert(codes.map(code => ({ code, created_by: user.id })))
  ...
  async function searchUsers(query) {
    ...
    const { data } = await supabase
      ...
      .neq('id', userId).limit(5)
    setSearchResults(data || [])
  }

  async function sendFollowRequest(targetId) {
    await supabase.from('follows').insert({ follower_id: userId, following_id: targetId, status: 'pending' })
    await supabase.from('notifications').insert({ recipient_id: targetId, actor_id: userId, type: 'follow_request' })
  ...
  // ── STEP 1: WELCOME ──
  if (step === 1) return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', maxWidth: '480px', margin: '0 auto' }}>
      <div style={{ background: 'linear-gradient(160deg, var(--clay) 0%, var(--ember) 60%, var(--parchment) 100%)', height: '45vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, padding: '32px 28px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div>
          <ProgressDots step={1} />
          <button onClick={() => setStep(2)} style={{ width: '100%', padding: '15px', background: 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '15px', fontWeight: '700', cursor: 'pointer', marginBottom: '10px' }}>Let's go</button>
          <button onClick={() => onComplete('login')} style={{ width: '100%', padding: '10px', background: 'none', border: 'none', fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--muted)', cursor: 'pointer' }}>I already have an account</button>
        </div>
      </div>
    </div>
  )

  // ── STEP 2: CREATE ACCOUNT (with photo step inline) ──
  if (step === 2) return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', maxWidth: '480px', margin: '0 auto', padding: '48px 28px 40px' }}>

      {showPhotoStep ? (
        // ── PHOTO STEP (inside step 2, survives re-render) ──
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <input id="onboarding-avatar" type="file" accept="image/*" style={{ display: 'none' }} onChange={handlePhotoUpload} />
          <label htmlFor="onboarding-avatar" style={{ cursor: 'pointer', marginBottom: '16px' }}>
            <div style={{ position: 'relative' }}>
              {avatarUrl ? (
                <img src={avatarUrl} alt="" style={{ width: '120px', height: '120px', borderRadius: '50%', objectFit: 'cover', boxShadow: '0 0 0 4px var(--cream), 0 0 0 6px var(--clay)' }} />
              ) : (
                <div style={{ width: '120px', height: '120px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--clay), var(--ember))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: '48px', fontWeight: '700', color: 'var(--cream)', boxShadow: '0 0 0 4px var(--cream), 0 0 0 6px var(--tan)' }}>
                  {(fullName || '?')[0].toUpperCase()}
                </div>
              )}
              <div style={{ position: 'absolute', bottom: '4px', right: '4px', width: '32px', height: '32px', borderRadius: '50%', background: 'var(--clay)', border: '3px solid var(--cream)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {avatarUploading
          ...
          <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '40px' }}>
            {avatarUploading ? 'Uploading...' : avatarUrl ? 'Tap to change' : 'Tap to add a photo'}
          </div>
          <div style={{ width: '100%' }}>
            <ProgressDots step={2} />
            <button onClick={() => { setShowPhotoStep(false); setStep(3) }} style={{ width: '100%', padding: '15px', background: 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '15px', fontWeight: '700', cursor: 'pointer', marginBottom: '10px' }}>
              Continue
            </button>
            {!avatarUrl && (
              <button onClick={() => { setShowPhotoStep(false); setStep(3) }} style={{ width: '100%', padding: '10px', background: 'none', border: 'none', fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--muted)', cursor: 'pointer' }}>
                Skip for now
              </button>
            )}
          </div>
        ...
        <>
          <button onClick={() => setStep(1)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--muted)', fontFamily: 'var(--font-body)', fontSize: '13px', fontWeight: '600', padding: '0 0 24px' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M19 12H5M5 12l7-7M5 12l7 7" stroke="var(--muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            Back
          </button>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" value={username} onChange={e => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} placeholder="alexcooks" style={{ ...inputStyle, paddingLeft: '28px' }} />
            </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <input type="text" value={inviteCode} onChange={e => setInviteCode(e.target.value.trim().toUpperCase())} placeholder="XXXXXXXX" style={{ ...inputStyle, letterSpacing: '0.1em' }} />
            <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '4px' }}>You need an invite code to join Nom.</div>
          </div>

          {authError && (
            <div style={{ background: '#FDE8E8', border: '1px solid #F5C0C0', borderRadius: 'var(--radius-md)', padding: '12px 16px', fontSize: '13px', color: '#B85252', marginBottom: '16px' }}>{authError}</div>
          )}

          <ProgressDots step={2} />
          <button onClick={handleCreateAccount} disabled={authLoading || !fullName || !username || !email || !password} style={{
  ...
  if (step === 3) return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', maxWidth: '480px', margin: '0 auto', padding: '48px 28px 40px' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--parchment)', borderRadius: 'var(--radius-md)', padding: '12px 16px', marginBottom: '16px', border: '1.5px solid var(--tan)' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7" stroke="var(--muted)" strokeWidth="1.8"/><path d="M16.5 16.5L21 21" stroke="var(--muted)" strokeWidth="1.8" strokeLinecap="round"/></svg>
          <input type="text" value={searchQuery} onChange={e => searchUsers(e.target.value)} placeholder="Search by name or username" style={{ flex: 1, border: 'none', background: 'none', fontFamily: 'var(--font-body)', fontSize: '14px', color: 'var(--ink)', outline: 'none' }} />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {searchResults.map(u => (
            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'var(--parchment)', borderRadius: 'var(--radius-md)', padding: '12px 14px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--clay), var(--ember))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: '700', color: 'var(--cream)', flexShrink: 0 }}>{(u.full_name || u.username || '?')[0].toUpperCase()}</div>
              <div style={{ flex: 1 }}>
              ...
              <button onClick={() => !requested[u.id] && sendFollowRequest(u.id)} style={{ padding: '7px 14px', background: requested[u.id] ? '#EEF4E5' : 'var(--clay)', color: requested[u.id] ? 'var(--forest)' : 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: '600', cursor: requested[u.id] ? 'default' : 'pointer' }}>{requested[u.id] ? 'Sent ✓' : '+ Request'}</button>
            </div>
          ))}
        </div>
      </div>
      <div>
        <ProgressDots step={3} />
        <button onClick={() => setStep(4)} style={{ width: '100%', padding: '15px', background: 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '15px', fontWeight: '700', cursor: 'pointer', marginBottom: '10px' }}>Continue</button>
        <button onClick={() => setStep(4)} style={{ width: '100%', padding: '10px', background: 'none', border: 'none', fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--muted)', cursor: 'pointer' }}>Skip for now</button>
      </div>
    </div>
  )
  ...
  if (step === 4) return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', maxWidth: '480px', margin: '0 auto', padding: '48px 28px 40px' }}>
      <button onClick={() => setStep(3)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--muted)', fontFamily: 'var(--font-body)', fontSize: '13px', fontWeight: '600', padding: '0 0 24px' }}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M19 12H5M5 12l7-7M5 12l7 7" stroke="var(--muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        Back
      </button>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <input type="url" value={recipeUrl} onChange={e => { setRecipeUrl(e.target.value); setImportedRecipe(null); setImportError(null) }} placeholder="https://nytcooking.com/recipe..." style={{ ...inputStyle, flex: 1 }} />
            <button onClick={importRecipe} disabled={importing || !recipeUrl.trim()} style={{ padding: '13px 16px', background: importing || !recipeUrl.trim() ? 'var(--tan)' : 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-body)', fontSize: '13px', fontWeight: '600', cursor: importing || !recipeUrl.trim() ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}>{importing ? '...' : 'Import'}</button>
          </div>
        </div>
        {importError && <div style={{ fontSize: '12px', color: '#B85252', marginBottom: '12px' }}>{importError}</div>}
        {importedRecipe && (
          <div style={{ background: 'var(--warm-white)', border: '1px solid var(--parchment)', borderRadius: 'var(--radius-md)', padding: '14px', marginTop: '12px' }}>
            {importedRecipe.image_url && <img src={importedRecipe.image_url} alt="" style={{ width: '100%', height: '140px', objectFit: 'cover', borderRadius: '8px', marginBottom: '10px' }} />}
      ...
      <div style={{ paddingTop: '24px' }}>
        <ProgressDots step={4} />
        <button onClick={importedRecipe ? saveFirstRecipe : () => setStep(5)} style={{ width: '100%', padding: '15px', background: 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '15px', fontWeight: '700', cursor: 'pointer', marginBottom: '12px' }}>
          {importedRecipe ? 'Save to Cookbook' : 'Skip for now'}
        </button>
        {importedRecipe && (
          <button onClick={() => setStep(5)} style={{ width: '100%', padding: '8px', background: 'none', border: 'none', fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--muted)', cursor: 'pointer' }}>I'll do this later</button>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', maxWidth: '480px', margin: '0 auto' }}>
      <div style={{ background: 'linear-gradient(160deg, var(--clay) 0%, var(--ember) 55%, var(--parchment) 100%)', height: '52vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        <svg width="280" height="220" viewBox="0 0 280 220" fill="none">
          ...
          <path d="M178 134 Q174 124 178 114" stroke="rgba(255,255,255,0.45)" strokeWidth="2.2" strokeLinecap="round" fill="none">
      ...
      <div style={{ flex: 1, padding: '32px 28px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '32px', fontWeight: '700', color: 'var(--ink)', lineHeight: '1.15', marginBottom: '10px' }}>Your Cookbook<br />is open.</div>
          <div style={{ fontSize: '14px', color: 'var(--muted)', lineHeight: '1.6' }}>Now go find something delicious.</div>
        </div>
        <div>
          <ProgressDots step={5} />
          <button onClick={finishOnboarding} style={{ width: '100%', padding: '15px', background: 'var(--clay)', color: 'var(--cream)', border: 'none', borderRadius: 'var(--radius-pill)', fontFamily: 'var(--font-body)', fontSize: '15px', fontWeight: '700', cursor: 'pointer' }}>Take me in</button>
        </div>
      </div>
    </div>
  )
